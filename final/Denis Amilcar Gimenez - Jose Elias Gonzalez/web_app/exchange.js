...
const exchange_name = 'POLISWAP';             // TODO: fill in the name of your exchange

const token_name = 'POLICOIN';                // TODO: replace with name of your token
const token_symbol = 'PCN';              // TODO: replace with symbol for your token
...
async function init() {
    ...
    if (poolState['token_liquidity'] === 0
      ...
      // permite al contrato del exchange gastar tokens del usuario hasta un total de total_supply
...
async function getPoolState() {
    ...
    // obtiene todas las cuentas de la red
    let accounts = await provider.listAccounts()
    // obtiene las reservas del token en el exchange, no importa con que cuenta se conecte al exchange realmente
...
async function addLiquidity(amountEth, maxSlippagePct) {
  ...
  await exchangeWithSigner.addLiquidity({
...
async function removeLiquidity(amountEth, maxSlippagePct) {
  ...
  await exchangeWithSigner.removeLiquidity(ethers.utils.parseEther(amountEth));
}
...
async function removeAllLiquidity(maxSlippagePct) {
  ...
  await exchangeWithSigner.removeAllLiquidity();
}
...
async function swapTokensForETH(amountToken, maxSlippagePct) {
  ...
  // obtiene la cuenta del usuario
    const signer = provider.getSigner(defaultAccount);
    // conecta el contrato del exchange con el usuario
    const exchangeWithSigner = exchange_contract.connect(signer);
    // conecta el contrato del token con el usuario
    const tokenWithSigner = token_contract.connect(signer);
    // obtiene la cantidad de tokens que el usuario ha permitido al exchange gastar
    const allowance = await tokenWithSigner.allowance(defaultAccount, exchange_address);
    // si la cantidad de tokens que el usuario ha permitido al exchange gastar es menor a la cantidad de tokens que el usuario quiere gastar
    if (allowance.lt(amountToken)) {
        // permite al exchange gastar la cantidad de tokens que el usuario quiere gastar
        const tx = await tokenWithSigner.approve(exchange_address, amountToken);
        // espera a que la transacción se complete
        await tx.wait();
    }
    // intercambia tokens por ETH
    await exchangeWithSigner.swapTokensForETH(amountToken);
}
...
async function swapETHForTokens(amountEth, maxSlippagePct) {
  ...
  // conecta el contrato del exchange con el usuario
  const exchangeWithSigner = exchange_contract.connect(signer);
  // intercambia ETH por tokens
  await exchangeWithSigner.swapETHForTokens({
      value: ethers.utils.parseEther(amountEth)
  });
}
...
async function updateAssetsInfo() {
  ...
  $("#TokenAssets").html(await token_contract.connect(provider.getSigner(defaultAccount)).balanceOf(defaultAccount) + " " + token_symbol);
  $("#ETHAssets").html(Math.round(await ethers.utils.formatUnits(await provider.getBalance(defaultAccount))) + " ETH");
}
...
// Changes the default account when you select a new account
$("#myaccount").change(updateAssetsInfo);
...
const sanityCheck = async function() {
  ...
  if (Number(swap_fee[0]) == 0) {
      ...
      console.log(`user_tokens2: ${Number(user_tokens2)}`)
