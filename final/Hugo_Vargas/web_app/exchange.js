...
const token_name = 'endo';                // TODO: replace with name of your token
const token_symbol = 'END';              // TODO: replace with symbol for your token
...
async function addLiquidity(amountEth, maxSlippagePct) {
  let defaultAccount = $("#myaccount").val(); //sets the default account
  let reserves_rates = await getPoolState();
  let account = provider.getSigner(defaultAccount)
  let endos = Math.round(amountEth * reserves_rates.token_eth_rate);
  await token_contract.connect(account).approve(exchange_address, endos);
  await exchange_contract.connect(account).addLiquidity({
...
async function removeLiquidity(amountEth, maxSlippagePct) {
  ...
  await exchange_contract.connect(account).removeLiquidity(ethers.utils.parseEther(amountEth));
}
...
async function removeAllLiquidity(maxSlippagePct) {
  ...
  await exchange_contract.connect(account).removeAllLiquidity();
}

/*** SWAP ***/
async function swapETHForTokens(amountToken, maxSlippagePct) {
  let defaultAccount = $("#myaccount").val(); //sets the default account
  let account = provider.getSigner(defaultAccount)

  await exchange_contract.connect(account).swapETHForTokens(amountToken);
}

async function swapTokensForETH(amountEth, maxSlippagePct) {
  let defaultAccount = $("#myaccount").val(); //sets the default account
  let account = provider.getSigner(defaultAccount)

  await exchange_contract.connect(account).swapTokensForETH({value: ethers.utils.parseEther(amountEth)});
}
