...
const exchange_name = 'block_exchange';

const token_name = 'BLOCKCHAIN';
const token_symbol = 'BLCK';
...
async function init() {
    ...
    else console.log("Pool already initialized");
}
...
async function addLiquidity(amountEth, maxSlippagePct) {
  const value_eth = ethers.utils.parseEther(amountEth);
  poolState = await getPoolState();
  const value_token = Math.ceil(amountEth * poolState.token_eth_rate);
  await token_contract.connect(provider.getSigner(defaultAccount)).approve(exchange_address, value_token);
  await exchange_contract.connect(provider.getSigner(defaultAccount)).addLiquidity({ value: value_eth });
}

/*** REMOVE LIQUIDITY ***/
async function removeLiquidity(amountEth, maxSlippagePct) {
  const value = ethers.utils.parseEther(amountEth);
  await exchange_contract.connect(provider.getSigner(defaultAccount)).removeLiquidity(value);
}
...
async function swapTokensForETH(amountToken, maxSlippagePct) {
  const value = amountToken;
  await token_contract.connect(provider.getSigner(defaultAccount)).approve(exchange_address, value);
  await exchange_contract.connect(provider.getSigner(defaultAccount)).swapTokensForETH(value);
}

async function swapETHForTokens(amountEth, maxSlippagePct) {
  const value = ethers.utils.parseEther(amountEth);
  await exchange_contract.connect(provider.getSigner(defaultAccount)).swapETHForTokens({ value: value })
}

// muestra en consola los balances de la cuenta seleccionada
$(document).ready(function () {
  $("#myaccount").change(async function () {
    console.log($(this).val());
    const balance_eth = await provider.getBalance($(this).val());
    const balance_token = await token_contract.connect(provider.getSigner($(this).val())).balanceOf($(this).val());
    console.log("token: ", Number(balance_token));
    console.log("eth: ", ethers.utils.formatEther(balance_eth));
  });
});
