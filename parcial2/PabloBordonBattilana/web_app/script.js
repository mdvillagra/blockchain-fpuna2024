...
async function getUsers() {
    let users = new Set();
    let events = await getAllFunctionCalls(contractAddress, "add_IOU");

    for (let i = 0; i < events.length; i++) {
        users.add(events[i].from);
        users.add(events[i].args[0]);
    }
...
async function getTotalOwed(user) {
    ...
    for (let i = 0; i < users.length; i++) {
        let amount = await BlockchainSplitwise.lookup(user, users[i]);
        totalOwed += parseInt(amount);
    }
...
async function getLastActive(user) {
    ...
    for (let i = events.length - 1; i >= 0; i--) {
        if (events[i].from.toLowerCase() === user.toLowerCase() || events[i].args[0].toLowerCase() === user.toLowerCase()) {
            return events[i].t;
        }
    }
...
async function add_IOU(creditor, amount) {
    let tx = await BlockchainSplitwise.add_IOU(creditor, amount);
    await tx.wait();
}
